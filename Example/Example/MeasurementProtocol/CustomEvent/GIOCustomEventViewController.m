...
- (IBAction)trackCustomEvent:(id)sender {
    ...
    if (atts.count > 0) {
        ...
        [[GrowingSDK sharedInstance] trackCustomEvent:eventName withAttributes:builder.build];
        NSLog(@"Track事件，eventName:%@, attributes:%@", eventName, atts);
#elif defined(SDK2nd)
        [Growing track:eventName withVariable:atts];
#endif
    } else {
#if defined(SDK3rd)
...
#elif defined(SDK2nd)
        [Growing track:eid withVariable:tvar];
#endif
}
